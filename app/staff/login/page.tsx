...
export default function StaffLogin() {
  ...
  async function submit(e:FormEvent){e.preventDefault();setBusy(true);setError('');const r=await fetch('/api/staff/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({account_id:accountId,employee_code:code,credential,work_type:workType,location_id:workType==='event'?null:locationId,event_id:workType==='event'?eventId:null,trusted_device:trusted})});const x=await r.json();if(!r.ok){setBusy(false);setError(x.error);return;}if(x.must_set_pin){router.push('/staff/setup-pin');return;}if(x.app_token_hash){const {error:authError}=await supabase.auth.verifyOtp({token_hash:x.app_token_hash,type:'magiclink'});if(authError){setBusy(false);setError('You were clocked in, but the app could not be opened. Ask an owner to confirm your Team and Staff profiles use the same email and role.');return;}window.location.replace('/dashboard');return;}router.push('/staff');}
