...
export async function POST(req: NextRequest) {
  ...
  const response = NextResponse.json({ success: true, must_set_pin: employee.must_set_pin, employee: { id: employee.id, full_name: employee.full_name, role: employee.role } });
...
export async function PUT(req: NextRequest) {
  ...
  const { data: employee } = await supabaseAdmin.from('staff_employees').select('id,pin_hash,must_set_pin,is_active,credential_version').eq('id', session.employeeId).eq('account_id', session.accountId).maybeSingle();
